function [DSGCpopResp_subType,alignJitter,tuningWidths,onr_idx] = occSim_2D_speed(PD,onr,Ncells,Tbins,Ktrials,dt,...
...
DSGCpopResp_subType_noise = zeros(Ncells,Tbins,Ktrials); % Background firing noise

% Generate uniform random numbers between 0 and 1
firingProb = rand(Ncells,Tbins,Ktrials);
DSGCpopResp_subType_noise(firingProb<baselineFR*dt) = 1/dt;
...
spikeRespDuration = speed_adjustDuration_ff(spikeRespDuration,...
    barSpeed,barSpeed_baseline,speed_duration_scalingFactor_ff); %added 1-10-20

%spikeRespDuration(ff_idx) = spikeRespDuration; %added 1-10-20

% (3) Convert onset times to receptive field (RF) radii (Nx1)
spatialOnset_ff = speed_adjustOnset(0,spikeRespOnset,...  % Convert temporal to spatial onset
    barSpeed,barSpeed_baseline,onset_speed_slope_ff,...
    radius_movingField);   % Adjust for current bar speed
radius_RF = radius_movingField - spatialOnset_ff;       % Measure from soma rather than periphery
...
if onr  % Occlusion null response
    ...
    spatialOnset_onr = speed_adjustOnset(onr,spikeRespOnsetONR,... 
        barSpeed,barSpeed_baseline,onset_speed_slope_onr,radius_movingField);
    radius_RF(onr_idx) = spatialOnset_onr;
    
    spikeRespOnset(onr_idx) = spikeRespOnsetONR;
    
    spikeRespDurationONR = speed_adjustDuration_onr(...
        spikeRespDurationONR,barSpeed,barSpeed_baseline,...
        speed_duration_scalingFactor_onr);
...
if ~alignPD     % Jitter in PD axis alignment?
    [spikeRespPeakFR,alignJitter,tuningWidths] = jitterPDalignment(Ncells,alignMargin,spikeRespPeakFR,...
...
%sinFit_param = NaN(Ncells,2);               % Initialize
...
DSGCpopResp_subType = DSGCpopResp_subType + DSGCpopResp_subType_noise;
end
...
function [fr,alignJitter,tuningWidths] = jitterPDalignment(Ncells,alignMargin,spikeRespPeakFR,...
    tuningWidth_gaussFit_param,gaussFit_collapsedTuningCurve_SD)
%{
OUTPUTS:
    fr           = 1xN (Hz)
    alignJitter  = 1xN (deg)
    tuningWidths = 1xN (deg)
%}
...
alignAngDev = sqrt(abs(2*alignCircVar));
